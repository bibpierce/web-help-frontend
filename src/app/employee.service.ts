...
export class EmployeeService {
  ...
  public updateEmployee(employee: Employee): Observable<any> {
    ...
    return this.http.put(url, employee, this.httpOptions).pipe(
      tap(_ => this.utilService.log(`updated employee id=${employee.id}`)),
      catchError(this.utilService.handleError<any>('updateEmployee'))
    )
  }

  public assignEmployee(ticketNumber: number, employeeNumber: number,): Observable<any> {
    let params = new HttpParams()
    params = params.set('employeeNumber', employeeNumber);
    const url = `${this.employeeUrl}/assign/${ticketNumber}`;
    return this.http.patch(url, {}, {params: params});
  }
